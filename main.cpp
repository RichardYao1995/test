...
//    rtabmap::StereoCameraModel model = node.sensorData().stereoCameraModel();
    ...
    cv::Mat img1 = cv::imread("/home/uisee/workspace/rtabmap/bin/image/1.tiff");
    cv::Mat img2 = cv::imread("/home/uisee/workspace/rtabmap/bin/image/2.tiff");
    right = img1(cv::Rect(img1.cols / 2, 0, img1.cols / 2, img1.rows));
    color = img1(cv::Rect(0, 0, img1.cols / 2, img1.rows));
    cv::cvtColor(color, gray, CV_BGR2GRAY);
    cv::cvtColor(right, right, CV_BGR2GRAY);
    disparity = rtabmap::util2d::disparityFromStereoImages(gray, right);
    ...
    color = img2(cv::Rect(0, 0, img2.cols / 2, img2.rows));
    cv::Mat gray1;
    ...
    for(Measurement m:measurements)
    {
        ...
        cv::circle(img_show, cv::Point2d(pixel_now(0, 0), pixel_now(1, 0) + color.rows), 4,
                   cv::Scalar(b, g, r), 2);
        }
